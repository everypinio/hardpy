...
def test_dut_serial_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_dut_serial_number():
            ...
            with pytest.raises(DuplicateParameterError):
                jig.set_dut_serial_number(second_serial_number)
    """,
    )
...
def test_dut_part_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_empty_dut_serial_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_clean_dut_serial_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_dut_info(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_dut_info():
...
def test_empty_dut_info(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_dut_sub_unit(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import set_dut_sub_unit, SubUnit
        ...
        def test_sub_unit():
            ...
            jig.set_dut_sub_unit(sub_unit_2)
            report = jig.get_current_report()
...
def test_stand_name(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_stand_location(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_stand_duplicate_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_stand_number():
            ...
            stand_number = 1
            jig.set_stand_number(stand_number)
            report = jig.get_current_report()
            assert stand_number == report.test_stand.number

            second_number = 2
            with pytest.raises(DuplicateParameterError):
                jig.set_stand_number(second_number)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_stand_incorrect_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import TestStandNumberError
...
def test_stand_info(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_stand_info():
...
def test_drivers(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_drivers():
...
def test_empty_drivers_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_empty_drivers_data():
            report_before = jig.get_current_report()
            drivers = {{}}
            jig.set_driver_info(drivers)
            report_after = jig.get_current_report()
...
def test_incorrect_drivers_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_module_artifact(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_module_artifact(request):
            ...
            jig.set_module_artifact(artifact_data)
            report = jig.get_current_report()
...
def test_empty_module_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_incorrect_module_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_case_artifact(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_case_artifact(request):
            ...
            jig.set_case_artifact(artifact_data)
            report = jig.get_current_report()
...
def test_empty_case_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_incorrect_case_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_run_artifact(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_empty_run_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_incorrect_run_artifact_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_run_artifact(request):
...
def test_case_message(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_message(request):
            ...
            key_3 = "msg_3"
            jig.set_message(msg_3, msg_key=key_3)
            report = jig.get_current_report()
            msgs = report.modules[node.module_id].cases[node.case_id].msg
            ...
            jig.set_message(msg_4, msg_key=key_3)
            report = jig.get_current_report()
            msgs = report.modules[node.module_id].cases[node.case_id].msg
...
def test_empty_case_message(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_message(request):
...
def test_operator_message(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_operator_message(request):
            ...
            with pytest.raises(ValueError):
                jig.set_operator_message(msg=None)

            with pytest.raises(ValueError):
                jig.set_operator_message(msg="a", font_size=0)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_block_operator_message(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        @pytest.mark.timeout(2)
        def test_block_operator_message():
            jig.set_operator_message(msg="a")
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(failed=1)
...
def test_non_block_operator_message(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        @pytest.mark.timeout(2)
...
def test_incorrect_couchdbconfig_data(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_jig_start_arg(pytester: Pytester, jig_opts: list[str]):
    """Test that --jig-start-arg passes key=value arguments correctly."""
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_start_arg_access(request, jig_start_args):
...
def test_user_name(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_batch_serial_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_batch_serial_number():
            ...
            jig.set_batch_serial_number(serial_number)
            report = jig.get_current_report()
            assert serial_number == report.batch_serial_number

            second_serial = "BATCH-456"
            with pytest.raises(DuplicateParameterError):
                jig.set_batch_serial_number(second_serial)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_dut_name(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_dut_type(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_dut_type():
            ...
            dut_type = "PCBA"
            jig.set_dut_type(dut_type)
            report = jig.get_current_report()
            assert dut_type == report.dut.type

            second_type = "Module"
            with pytest.raises(DuplicateParameterError):
                jig.set_dut_type(second_type)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_dut_revision(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_dut_revision():
            ...
            revision = "REV1.0"
            jig.set_dut_revision(revision)
            report = jig.get_current_report()
            assert revision == report.dut.revision

            second_rev = "REV2.0"
            with pytest.raises(DuplicateParameterError):
                jig.set_dut_revision(second_rev)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_stand_revision(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_stand_revision():
            ...
            revision = "HW1.0"
            jig.set_stand_revision(revision)
            report = jig.get_current_report()
            assert revision == report.test_stand.revision

            second_rev = "HW2.0"
            with pytest.raises(DuplicateParameterError):
                jig.set_stand_revision(second_rev)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_instruments(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_instruments():
            ...
            jig.set_instrument(instrument1)
            ...
            jig.set_instrument(instrument2)
...
def test_process_name(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
...
def test_process_number(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        from jig import DuplicateParameterError
        ...
        def test_process_number():
            ...
            number = 1
            jig.set_process_number(number)
            report = jig.get_current_report()
            assert number == report.process.number

            second_number = 2
            with pytest.raises(DuplicateParameterError):
                jig.set_process_number(second_number)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_process_info(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
...
def test_numeric_measurement(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_measurement(request):
            ...
            meas1 = jig.NumericMeasurement(value=1)
            index_1 = jig.set_case_measurement(meas1)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
            ...
            meas2 = jig.NumericMeasurement(
                value=2,
                operation=jig.ComparisonOperation.EQ,
                comparison_value=2
            )
            index_2 = jig.set_case_measurement(meas2)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
            ...
            meas3 = jig.NumericMeasurement(
                value=5,
                operation=jig.ComparisonOperation.GTLT,
                lower_limit=1,
                upper_limit=4
            )
            index_3 = jig.set_case_measurement(meas3)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
...
def test_string_measurement(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_measurement(request):
            ...
            meas1 = jig.StringMeasurement(value="a")
            jig.set_case_measurement(meas1)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
            assert len(measurements) == 1
            ...
            meas2 = jig.StringMeasurement(
                value="b",
                operation=jig.ComparisonOperation.EQ,
                comparison_value="b"
            )
            jig.set_case_measurement(meas2)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
            assert len(measurements) == 2
            ...
            meas3 = jig.StringMeasurement(
                value="A",
                operation=jig.ComparisonOperation.NE,
                casesensitive=False,
                comparison_value="a"
            )
            jig.set_case_measurement(meas3)
            report = jig.get_current_report()
            measurements = report.modules[module_id].cases[case_id].measurements
            assert len(measurements) == 3
...
def test_set_case_chart(pytester: Pytester, jig_opts: list[str]):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_set_case_chart(request):
            ...
            with pytest.raises(jig.DuplicateParameterError):
                jig.set_case_chart(chart_2)
    """,
    )
    result = pytester.runpytest(*jig_opts)
    result.assert_outcomes(passed=1)
...
def test_get_current_attempt(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        f"""{func_test_header}

        def test_a():
            assert jig.get_current_attempt() == 1
...
def test_get_jig_config(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        f"""{func_test_header}
