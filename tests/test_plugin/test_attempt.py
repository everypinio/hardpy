...
def test_attempt_success(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        test_1=f"""{func_test_header}

        from jig.pytest_jig.utils.const import TestStatus
...
def test_attempt_failed(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        test_1=f"""{func_test_header}
...
def test_critical_with_attempt_all_failed(pytester: Pytester, jig_opts: list[str]):
...
def test_critical_with_attempt_dependency(pytester: Pytester, jig_opts: list[str]):
...
def test_error_code_empty_after_success_attempt(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        f"""
        {func_test_header}
        @pytest.mark.attempt(2)
...
def test_success_attempt_after_error_code(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        f"""
        {func_test_header}
        def test_a():
...
def test_different_error_code_some_attempt(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        f"""{func_test_header}
        @pytest.mark.attempt(2)
        def test_1():
            attempt = jig.get_current_attempt()
            assert False, jig.ErrorCode(attempt)

        def test_2():
            assert False, jig.ErrorCode(3)
...
def test_clear_case_data(pytester: Pytester, jig_opts: list):
    pytester.makepyfile(
        test_1=f"""{func_test_header}
        @pytest.mark.attempt(2)
        def test_a():
            ...
            meas = jig.NumericMeasurement(value=1)
            jig.set_case_measurement(meas)
