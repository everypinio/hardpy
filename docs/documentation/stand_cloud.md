...
## StandCloud and Jig integration

**Jig** allows test result data to be stored in the **StandCloud** and
read them from the **StandCloud**.
For an example of StandCloud and Jig integration,
...
instead of being stored in the `jig.toml` configuration file.
The `.env` file must be located in the root of the working project.
The variable name is `JIG_SC_API_KEY`.
An example file is available in the root of the **Jig** repository
under the name `.env.example`.
If the `JIG_SC_API_KEY` environment variable is defined,
its value takes priority over the `api_key` value
specified in the `jig.toml` file.

Example of the `.env` file content:

```bash
JIG_SC_API_KEY=your_api_key_here
```
...
1. Use the [set_dut_serial_number](./pytest_jig.md#set_dut_serial_number)
...
2. Use the [set_dut_part_number](./pytest_jig.md#set_dut_part_number)
   to store DUT part number.
   The part number is an identifier that allows you to clearly 
   identify the product.   
3. Use the [set_case_artifact](./pytest_jig.md#set_case_artifact),
   [set_module_artifact](./pytest_jig.md#set_module_artifact),
   and [set_run_artifact](./pytest_jig.md#set_run_artifact)
   to store important information, that cannot be accommodated in other structured fields
   such as [set_case_measurement](./pytest_jig.md#set_case_measurement)
   and [set_case_chart](./pytest_jig.md#set_case_measurement).
4. Use the [case_name](./pytest_jig.md#case_name)
   and [module_name](./pytest_jig.md#module_name)
