...
**Jig** uses the `jig.toml` file for configuration.
The user can change the fields at creation by using [jig init](./cli.md#jig-init).

???+ note
    All **Jig** project must have `jig.toml` file.
...
The value is always `Jig TOML config`.

#### tests_name

Tests name. The user can change this value with the `jig init --tests-name` argument.
...
Files are stored in the `.jig` directory in the root of the project.
The user can change this value with the `jig init --storage-type` option.

#### storage_path

Path to the storage directory. The default is `.jig` in the root of the project.
The user can change this value in the jig.toml file using the `storage_path` option. 
...
The user can change this value with the `jig init --database-user` option.

#### password

Database password. The default is `dev`.
The user can change this value with the `jig init --database-password` option.

#### host

Database host name. The default is `localhost`.
The user can change this value with the `jig init --database-host` option.

#### port

Database port number. The default is `5984`.
The user can change this value with the `jig init --database-port` option.
...
The user can change this value with the `jig init --frontend-host` option.

#### port

Operator panel port number. The default is `8000`.
The user can change this value with the `jig init --frontend-port` option.

#### language

Language of operator panel. The default is `en`.
Available languages are [there](jig_panel.md#languages).
...
Enable or disable manual test collection mode in the [operator panel](./jig_panel.md#manual-test-selection).
...
Boolean variable, if set to `true`, **Jig** will check the connection
...
Boolean variable, if set to `true`, **Jig** will automatically send data 
...
Users can add their own fields to the jig.toml file and use them in test plans. 
