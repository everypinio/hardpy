...
1. Create three separate projects, each with a different frontend port.
   Launch: 
    ...
    ```bash
    jig init tests_2 --frontend-port 8001
    ```

    ```bash
    jig init tests_3 --frontend-port 8002
    ```
...
Three instances of **Jig** will launch at the following addresses: 
...
Each document is synchronized with its corresponding Jig server address.
If the host addresses in the jig.toml file change, the document addresses will change as well.
