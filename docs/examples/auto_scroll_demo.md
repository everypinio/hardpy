...
This example demonstrates the optional `auto_scroll` flag in `jig.toml`.
...
[auto_scroll_demo](https://github.com/everypinio/jig/tree/main/examples/auto_scroll_demo).

### how to start

1. Launch `jig init auto_scroll_demo`.
2. Launch [CouchDB instance](../documentation/database.md#couchdb-instance).
3. Set `auto_scroll = true` under `[frontend]` in `jig.toml`.
4. Launch `jig run auto_scroll_demo`.
...
- **Burst** — one case that streams 42 measurements at 0.5 s intervals.
  ...
  `measurement_display = true` in `jig.toml` (the default), each
