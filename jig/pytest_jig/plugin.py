...
def pytest_addoption(parser: Parser) -> None:
    ...
    parser.addoption(
        "--jig-tests-name",
        action="store",
        help="Jig tests suite name",
    )
    # TODO (xorialexandrov): Remove --jig-sp and --jig-sh in Jig major version.
    ...
    parser.addoption(
        "--jig-config-file",
        action="store",
        help="Jig configuration file path",
    )
    parser.addoption(
...
class JigPlugin:
    """Jig integration plugin for pytest.
    ...
    def pytest_configure(self, config: Config) -> None:
        ...
        if jig_config_path:
            jig_config = config_manager.read_config(Path(jig_config_path))
        else:
            jig_config = config_manager.read_config(Path(config.rootpath))

        if not jig_config:
            jig_config = JigConfig()

        database_url = config.getoption("--jig-db-url")
        if database_url:
            jig_config.database.url = str(database_url)  # type: ignore

        tests_name = config.getoption("--jig-tests-name")
        if tests_name:
            self._tests_name = str(tests_name)
        else:
            self._tests_name = str(PurePath(config.rootpath).name)

        current_test_config = config.getoption("--jig-current-test-config")
        ...
        if sc_address:
            jig_config.stand_cloud.address = str(sc_address)  # type: ignore

        sc_connection_only = config.getoption("--sc-connection-only")
        if sc_connection_only:
            jig_config.stand_cloud.connection_only = bool(sc_connection_only)  # type: ignore

        sc_autosync = config.getoption("--sc-autosync")
        if sc_autosync:
            jig_config.stand_cloud.autosync = bool(sc_autosync)  # type: ignore

        _args = config.getoption("--jig-start-arg") or []
        if _args:
        ...
        if config_manager.config.stand_cloud.connection_only:
            ...
            try:
                ...
                msg = (
                    ...
                    "not available or Jig user is not authorized"
                )
                self._reporter.set_alert(msg)
                self._reporter.update_db_by_doc()
                exit(msg, ExitCode.INTERNAL_ERROR)
    ...
    @fixture(scope="session")
    def jig_start_args(self) -> dict:
        """Get Jig start arguments.
