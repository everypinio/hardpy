...
class StorageType(str, Enum):
    """Storage backend types for Jig data persistence.
...
class DatabaseConfig(BaseModel):
    ...
    storage_path: str = Field(exclude=True, default=".jig")
...
class JigConfig(BaseModel, extra="allow"):
    """Jig configuration."""

    model_config = ConfigDict(extra="allow")

    title: str = "Jig TOML config"
    tests_name: str = ""
...
class ConfigManager(metaclass=SingletonMeta):
    """Jig configuration manager."""

    def __init__(self) -> None:
        self._config = JigConfig()
        self._tests_path = Path.cwd()

    @property
    def config(self) -> JigConfig:
        """Get Jig configuration.
    ...
    def init_config(  # noqa: PLR0913
        ...
        """Initialize the Jig configuration.
    ...
    def create_config(self, parent_dir: Path) -> None:
        ...
        with Path.open(parent_dir / "jig.toml", "w") as file:
            file.write(config_str)

    def read_config(self, toml_path: Path) -> JigConfig | None:
        """Read Jig configuration.

        Args:
            toml_path (Path): jig.toml file path.

        Returns:
            JigConfig | None: Jig configuration
        """
        self._tests_path = toml_path
        toml_file = toml_path / "jig.toml"
        if not toml_file.exists():
            # TODO (xorialexandrov): Add a log that cannot cause tests to fail
            return None
        try:
            with Path.open(toml_path / "jig.toml", "rb") as f:
        ...
        try:
            self._config = JigConfig(**toml_data)
        ...
        sc_api_key = getenv("JIG_SC_API_KEY")
