...
from jig.common.config import JigConfig, StorageType
...
def _json_storage_dir(config: JigConfig, tests_path: Path) -> Path:
...
def _standcloud_status(config: JigConfig) -> str:
...
def _overall_storage_status(config: JigConfig, standcloud_status: str) -> str:
    storage_menu = config.frontend.reports_storage_menu
...
def _local_database_status(config: JigConfig, couchdb_available: bool) -> str:
