...
describe("ProgressView", () => {
 ...
 then the bar pulses and is coloured as primary`, () => {
 ...
 when the progress is rendered,
 then the bar stops pulsing`, () => {
    const { container } = render(<ProgressView percentage={100} status="run" />);

    expect(indicator(container)).not.toHaveClass("animate-pulse");
  });

  test(`given a passed run,
 when the progress is rendered,
 then the bar is coloured as success and does not pulse`, () => {
    const { container } = render(
      <ProgressView percentage={100} status="passed" />
    );

    expect(indicator(container)).toHaveClass(STATUS_COLORS.ok.fill);
    expect(indicator(container)).not.toHaveClass("animate-pulse");
  });
