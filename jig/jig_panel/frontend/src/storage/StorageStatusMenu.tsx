...
const OVERALL_STATUS_ICON_COLORS: Record<
  ...
  string
> = {
  standcloud_ready: STATUS_COLORS.ok.text,
  standcloud_needs_attention: "text-destructive",
  local_database_only: STATUS_COLORS.ok.text,
  files_only: STATUS_COLORS.ok.text,
...
const STANDCLOUD_API_KEYS_URL =
  "https://standcloud.everypin.io/dashboard/organization-profile/organization-api-keys?utm_source=jig_UI";
const COUCHDB_DOCS_URL =
  "https://everypinio.github.io/jig/documentation/database/#couchdb-instance";
const DEFAULT_COUCHDB_HOST = "localhost";
const DEFAULT_COUCHDB_PORT = 5984;
const PATH_COPIED_TOAST_DURATION_MS = 2000;

interface JigStorageMenuConfig {
  database?: {
    host?: string;
    port?: number;
...
const isStandCloudVisible = (
  jigConfig: JigStorageMenuConfig | null
): boolean =>
  jigConfig?.frontend?.reports_storage_menu?.show_standcloud ?? true;

const isStandCloudCheckEnabled = (
  jigConfig: JigStorageMenuConfig | null
): boolean =>
  jigConfig?.frontend?.reports_storage_menu?.check_standcloud ?? true;

const getStorageType = (
  jigConfig: JigStorageMenuConfig | null
): "couchdb" | "json" => jigConfig?.database?.storage_type ?? "couchdb";
...
const hasStorageProblem = (
  ...
  const standcloudProblem =
    isStandCloudVisible(jigConfig) &&
    isStandCloudCheckEnabled(jigConfig) &&
    data.standcloud.status !== "configured";
  const localDatabaseProblem =
    getStorageType(jigConfig) === "couchdb" &&
...
const getMenuIconColor = (
  ...
  if (hasStorageProblem(data, jigConfig)) {
...
const StorageStatusContent = ({
  ...
  const isFilesBackend = getStorageType(jigConfig) === "json";
  const standcloudVisible = isStandCloudVisible(jigConfig);
  const standcloudCheckEnabled = isStandCloudCheckEnabled(jigConfig);
  const localDatabaseConfigured = getStorageType(jigConfig) === "couchdb";
  const LocalBackendIcon = isFilesBackend ? FileText : Database;
  const localBackendIconColor = isFilesBackend
    ? "text-muted-foreground"
    : data.local_database.status === "connection_failed"
      ? "text-destructive"
      : STATUS_COLORS.ok.text;
  ...
  const couchDbPanelUrl = `http://${
    jigConfig?.database?.host ?? DEFAULT_COUCHDB_HOST
  }:${jigConfig?.database?.port ?? DEFAULT_COUCHDB_PORT}/_utils/`;
...
const StorageStatusMenu = ({
  ...
  const { t } = useTranslation();
  const statusColor = getMenuIconColor(data, loading, error, jigConfig);
